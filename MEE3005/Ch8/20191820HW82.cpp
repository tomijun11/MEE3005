...
void Gauss_Seidel(int ibeg, int iend, int jbeg, int jend, int maxiter, double maxerr, double lambda, double T[ID][JD]);
...
int main(void)
{
	...
	//
	// boundary conditions
	for (i = 0; i <= m + 1; i++) { T[i][0] = 900.;  T[i][n + 1] = 0.; } // y=0, y=n		
	for (j = 0; j <= n + 1; j++) { T[0][j] = 900.;  T[m + 1][j] = 0.; } // x=0, x=n
	//
	// solver
	Gauss_Seidel(1, m+1, 1, n+1, maxiter, maxerr, lambda, T);
	//
	...
	Plot_Contour(fp1, 20.0, 0, m + 1, 0, n + 1, x, y, T);
	Plot_Contour(fp1, 40.0, 0, m + 1, 0, n + 1, x, y, T);
		...
		for (j = jbeg; j <= jend; j++) {
			for (i = ibeg; i <= iend; i++) {
				...
				if (j == jend) // j = n
				{
					if (i == iend) // i = n
					{
						Tnew = (2. * T[i - 1][j] + 2. * T[i][j - 1] - 4. * h * pow(T[i][j], 3)) / 4.;
					}
					else
					{
						Tnew = (2. * T[i][j - 1] + T[i - 1][j] + T[i + 1][j] - 2. * h * pow(T[i][j], 3)) / 4.;
					}
				}
				else {
					if (i == iend) // i = n
					{
						Tnew = (2. * T[i - 1][j] + T[i][j - 1] + T[i][j + 1] - 2. * h * pow(T[i][j], 3)) / 4.;
					}
					else
					{
